...
export function renderAdminPage(pathname = '/', config: AdminPageConfig = { turnstileSiteKey: null }): string {
    ...
    ? `<script>
      (() => {
        ...
        window.addEventListener('load', () => {
          navigator.serviceWorker.register('${ownerPath}/edgegist-sw.js', { scope: '${ownerPath}/' }).catch(() => {});
        });
      })();
    </script>`
    : ''
...
function renderAdminBootMarkup(pathname: string): string {
  ...
  return `<div class="eg-boot" data-edgegist-boot-shell aria-busy="true" aria-label="Loading EdgeGist admin">
